...
<div class="col-md-9 body-container">
	...
	<div class="mdl-card mdl-shadow--4dp">
		<div class="mdl-card__title mdl-color--green-900">
			<h1 class="mdl-card__title-text text-center mdl-color-text--yellow-300">
				Subject Schedule
			</h1>
		</div>
					...
					@foreach ($days as $day)
						<tr>
							<td>
								<div class="checkbox">
									<label>
										<input type="checkbox" style="margin-top:8px;" name="day[]"
											   value="{{ $day->id }}" {{ set_checkbox('day', $day->id) }}>
								...
								<select class="form-control" name="start_time{{ $day->id }}">
									...
									@foreach ($times as $time)
										<option value="{{ $time->id }}"
												{{ set_select('start_time'.$day->id, $time->id) }}>
											{{ $time->time }} {{ ($time->id < 11) ? 'AM' : 'PM' }}
										</option>
									@endforeach
							...
							<td>
								<select class="form-control" name="end_time{{ $day->id }}">
									...
									@foreach ($times as $time)
										<option value="{{ $time->id }}" {{ set_select('end_time'.$day->id, $time->id) }}>
											{{ $time->time }} {{ ($time->id < 11) ? 'AM' : 'PM' }}
										</option>
									@endforeach
